...
- `DATABASE_URL`: PostgreSQL connection string (required; use an isolated DB/schema for benchmarks)
...
Ground truth: synthetic marker `[benchmark_doc_id=N]` appended to each support passage; Recall/NDCG use first hit in reranked top-10 contexts (see `docs/recall_evaluation.md`). Graph ingest skipped (`SKIP_GRAPH_INGEST=1`). Raw JSON: `recall_ndcg_benchmark_20260402T154831Z.json`.
