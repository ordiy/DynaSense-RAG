...
Enable tracing by setting (before the server imports ``rag_core``):

- ``LANGCHAIN_TRACING_V2=true`` (default when a key is present)
- ``LANGCHAIN_API_KEY`` — create at https://smith.langchain.com/settings

Optional:

- ``LANGCHAIN_PROJECT`` or ``LANGSMITH_PROJECT`` — project name in LangSmith UI

Never commit API keys; use ``.env`` or your secret manager (gitignored).
"""
